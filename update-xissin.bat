...
set "ADMIN_URL=https://xissin-admin.up.railway.app"
...
if "!NEW_VERSION!"=="!CURRENT_VERSION!" (
    echo.
    echo  ⚠️  That's the same as the current version!
...
:: ── Auto edit pubspec.yaml ───────────────────────────────────
powershell -Command "(Get-Content '%PUBSPEC%') -replace 'version: !CURRENT_VERSION!+!CURRENT_BUILD!', 'version: !NEW_VERSION!+!NEW_BUILD!' | Set-Content '%PUBSPEC%'"
...
if /i "!DONE2!" equ "N" (
    echo.
    echo  ⚠️  Please update it before continuing!
...
echo.
echo  ╔══════════════════════════════════════════════════╗
echo  ║   STEP 5 of 5 — Create Version Tag (Triggers    ║
echo  ║                  GitHub Actions Build)           ║
echo  ╚══════════════════════════════════════════════════╝
echo.
echo  🏷️  This will create tag: v!NEW_VERSION!
...
echo  🏷️  Running: git tag v!NEW_VERSION!
...
if %errorlevel% neq 0 (
    ...
    echo  Try a different version number.
    echo.
    pause
    exit /b
)
