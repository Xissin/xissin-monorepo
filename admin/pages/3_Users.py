...
with st.container(border=True):
    ...
    with col_x: target_uid = st.text_input("User ID", placeholder="Enter exact user_id...")
    with col_y: ban_reason = st.text_input("Reason (optional)", placeholder="e.g. Spamming")
    with col_z: action     = st.selectbox("Action", ["Ban","Unban"])
    col_btn, _ = st.columns([1,3])
    with col_btn:
        if st.button(f"{'🚫 BAN' if action=='Ban' else '✅ UNBAN'} USER", type="primary", use_container_width=True):
            if not target_uid.strip(): st.error("Enter a User ID.")
            else:
                try:
