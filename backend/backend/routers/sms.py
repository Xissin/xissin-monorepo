...
# Suppress SSL warnings for services with self-signed/broken certs on Railway
...
def _short_err(e: Exception) -> str:
    ...
    if "HTTPSConnectionPool" in msg or "HTTPConnectionPool" in msg:
        ...
        return "Connection failed (SSL/Network error)"
...
def _send_ezloan(phone: str):
    """EZLoan PH — OTP request via gateway."""
    try:
        p  = _format_phone(phone)
        ...
        if r.status_code in (200, 201):
            try:
                ...
                if rj.get("code") in (200, 201):
                    return True, "OTP sent"
                return False, (rj.get("msg") or rj.get("message") or f"Code {rj.get('code')}")[:50]
            except Exception:
                return True, "OTP sent"
        return False, f"HTTP {r.status_code}"
    except Exception as e:
...
def _send_xpress(phone: str):
    """Xpress PH — create user triggers OTP."""
    try:
        p   = _format_phone(phone)
        ts  = int(time.time())
...
def _send_mwell(phone: str):
    """MWELL — mobile number sign-in OTP."""
    try:
        p = _format_phone(phone)
        headers = {
